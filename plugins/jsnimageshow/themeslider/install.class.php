...
class plgjsnimageshowThemeSliderInstallerScript
{
	...
	private function _updateSchema()
	{
		...
		$eid = $row->find(array('element' => 'themeslider', 'type' => 'plugin'));
		if ($eid)
		{
			$db = JFactory::getDBO();
	...
	private function _getInfo($id)
	{
		...
		$query->where('element=\'themeslider\' AND type=\'plugin\' AND folder=\'jsnimageshow\' AND extension_id = ' . $id);
		$db->setQuery($query);
