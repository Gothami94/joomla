...
if ($saveOrder) {
	$saveOrderingUrl = 'index.php?option=com_flippingbook&task=pages.saveOrderAjax&tmpl=component';
...
<form action="<?php echo JRoute::_('index.php?option=com_flippingbook&view=pages');?>" method="post" name="adminForm" id="adminForm">
	<div class="row-fluid">
		<div id="filter-bar" class="btn-toolbar">
			<div class="filter-search btn-group pull-left">
					<label for="filter_search" class="element-invisible"><?php echo JText::_('COM_FLIPPINGBOOK_SEARCH_IN_FILE_NAME');?></label>
					<input type="text" name="filter_search" id="filter_search" placeholder="<?php echo JText::_('COM_FLIPPINGBOOK_SEARCH_IN_FILE_NAME'); ?>" value="<?php echo $this->escape($this->state->get('filter.search')); ?>" title="<?php echo JText::_('COM_FLIPPINGBOOK_SEARCH_IN_FILE_NAME'); ?>" />
				</div>
			...
			<table class="table table-striped" id="articleList">
				<thead>
					<tr>
						...
						<th width="20%">
							<?php echo JText::_('COM_FLIPPINGBOOK_PAGE_FILE'); ?>
						</th>
						<th width="20%">
							<?php echo JText::_('COM_FLIPPINGBOOK_ZOOMED_PAGE_FILE'); ?>
						</th>
						<th class="hidden-phone">
							<?php echo JText::_( 'COM_FLIPPINGBOOK_DESCRIPTION' ); ?>
						</th>
						<th width="10%" class="hidden-phone">
							<?php echo JText::_( 'COM_FLIPPINGBOOK_BUTTON_LINK' ); ?>
						</th>
						<th width="1%" class="center hidden-phone">
							<?php echo JText::_( 'COM_FLIPPINGBOOK_BOOK_ID' ); ?>
						</th>
						<th width="1%" class="center hidden-phone">
				...
				foreach ($this->items as $i => $item) :
					...
					<?php if ($canCreate || $canEdit) { ?>
						<a href="<?php echo JRoute::_( 'index.php?option=com_flippingbook&task=page.edit&id='. $item->id ); ?>" title="<?php echo JText::_( 'COM_FLIPPINGBOOK_EDIT_BOOK' ); ?>">
							<?php echo $this->escape($item->file); ?></a>
					<?php } else { 
							echo $this->escape($item->file);
						} ?>
					<div class="small">
						<?php echo JText::_('COM_FLIPPINGBOOK_BOOK') . ": " . $this->escape($item->book_title); ?>
					</div>
					</td>
					<td><?php echo $item->zoom_url; ?>
					</td>
					<td>
						<?php echo strip_tags($item->description); ?>
					</td>
					<td><?php echo $item->link_url; ?>
					</td>
					<td class="center">
						<?php echo $item->catid;?>
					</td>
